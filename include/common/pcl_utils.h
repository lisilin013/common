...
bool PointZAxisCompare(const PointT &lhs, const PointT &rhs) {
  static_assert(pcl::traits::has_xyz<PointT>::value,
                "PointT must be a structure containing XYZ coordinates");
  return lhs.z < rhs.z;
}
...
template <typename PointT>
PointT Vector4fToPoint3d(const Eigen::Vector4f &vec) {
  PointT pt;
  pt.x = vec.x();
  pt.y = vec.y();
  pt.z = vec.z();
...
template <typename PointT>
double ComputeXYDiameter(const PointT &point1, const PointT &point2) {
...
template <typename PointT>
float DistanceBetweenTwoPoints(const PointT &p1, const PointT &p2) {
